...
@php
    ...
    $selectedScope = $order->location->scope ?? 'rota';
    $selectedRouteId = $order->route_id;
    $selectedLocationId = $order->location_id;

    $existingItems = $order->items->map(function ($item) {
        return [
...
<div class="page-head">
    <div>
        ...
        <p>Atualize os itens do pedido com ajuste automático do estoque.</p>
    </div>
...
<form action="{{ route('orders.update', $order) }}" method="POST" class="card" id="formEditarPedido">
    @csrf
    @method('PUT')
    ...
    <div class="card-body">
        <div class="form-grid form-grid-order-top">
            <div class="form-group">
                ...
                <select name="scope_selector" id="scope" class="form-control-custom" required>
                    ...
                    <option value="rota" {{ $selectedScope === 'rota' ? 'selected' : '' }}>Rota</option>
                    <option value="almoxarifado" {{ $selectedScope === 'almoxarifado' ? 'selected' : '' }}>Almoxarifado</option>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="location_id" id="location_id" class="form-control-custom" required></select>
            </div>
        ...
        <div class="page-head" style="margin-bottom:14px;">
            <div>
                ...
                <p>Altere somente as quantidades necessárias.</p>
            </div>
        </div>
        ...
        <div class="actions-inline" style="margin-top:18px;">
            <button type="submit" class="btn btn-green">Atualizar Pedido</button>
...
<script>
    ...
    function getExistingQuantity(productId, variantId = null) {
    ...
    function getExistingQuantityNumber(productId, variantId = null) {
        const found = existingItems.find(function(item) {
            return String(item.product_id) === String(productId)
                && String(item.product_variant_id ?? '') === String(variantId ?? '');
        });
    ...
    function renderizarProdutos() {
        ...
        if (!scope) {
            wrapper.innerHTML = `<div class="card" style="padding:16px;"><div class="text-muted-small">Selecione primeiro o tipo do pedido para carregar os produtos.</div></div>`;
            return;
        }
        ...
        if (!produtosFiltrados.length) {
            wrapper.innerHTML = `<div class="card" style="padding:16px;"><div class="text-muted-small">Nenhum produto encontrado para este tipo.</div></div>`;
            return;
        }
        ...
        produtosFiltrados.forEach(function (produto) {
            if (!produto.uses_variants) {
                const qty = getExistingQuantity(produto.id, null);
            ...
            produto.variants.forEach(function (variant) {
                ...
                const qty = getExistingQuantity(produto.id, variant.id);
                const checkboxId = `produto_${produto.id}_variant_${variant.id}`;

                checksHtml += `
                    <label class="variant-check" for="${checkboxId}">
                        <input type="checkbox" id="${checkboxId}" onchange="toggleVariantRow(${produto.id}, ${variant.id})" ${checked ? 'checked' : ''}>
            ...
            wrapper.insertAdjacentHTML('beforeend', `
                <div class="card" style="padding:16px;">
                    ...
                    <div class="variant-picker">${checksHtml}</div>
                    <div class="variant-fields">${fieldsHtml}</div>
                </div>
            `);
        });
    }
    ...
    function validarEstoqueAntesDeEnviar() {
        ...
        document.querySelectorAll('.quantity-check-input').forEach(function(input) {
            ...
            const quantidadeNova = normalizarNumero(input.value);
            if (quantidadeNova <= 0) {
                return;
            }
            ...
            const estoqueDisponivelAgora = normalizarNumero(estoqueInput.value);

            const productHidden = linha.querySelector('input[name*="[product_id]"]');
            const variantHidden = linha.querySelector('input[name*="[product_variant_id]"]');

            const productId = productHidden ? productHidden.value : null;
            const variantId = variantHidden ? variantHidden.value : null;

            const quantidadeAntigaNoPedido = getExistingQuantityNumber(productId, variantId);
            const quantidadeMaximaPermitida = quantidadeAntigaNoPedido + estoqueDisponivelAgora;

            if (quantidadeNova > quantidadeMaximaPermitida) {
                temErro = true;

                adicionarErroCampo(
                    input,
                    'Estoque insuficiente. Você já tinha ' +
                    quantidadeAntigaNoPedido +
                    ' neste pedido e ainda restam ' +
                    estoqueDisponivelAgora +
                    ' no estoque. Máximo permitido: ' +
                    quantidadeMaximaPermitida + '.'
                );
            }
        });
    ...
    formEditarPedido.addEventListener('submit', function(e) {
        if (!validarEstoqueAntesDeEnviar()) {
            e.preventDefault();
            alert('Existem itens com quantidade maior que o estoque permitido para edição.');
        }
    });
    ...
    document.addEventListener('keydown', function(event) {
        ...
        if (!formEditarPedido.contains(target)) {
            return;
        }
        ...
        const confirmar = confirm('Deseja realmente atualizar este pedido?');

        if (!confirmar) {
            return;
        }

        formEditarPedido.requestSubmit();
    }, true);
